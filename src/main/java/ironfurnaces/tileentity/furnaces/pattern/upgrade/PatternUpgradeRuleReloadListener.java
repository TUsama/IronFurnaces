...
public final class PatternUpgradeRuleReloadListener extends SimpleJsonResourceReloadListener<JsonElement> {
    ...
    public PatternUpgradeRuleReloadListener() {
        ...
        super(ExtraCodecs.JSON, FileToIdConverter.json(PatternUpgradeRuleManager.DIRECTORY));
    }
    ...
    protected void apply(
        ...
        Map<Identifier, PatternUpgradeRule> loaded = new HashMap<>();

        int ok = 0;
        int failed = 0;

        for (Map.Entry<Identifier, JsonElement> e : jsonMap.entrySet()) {
            Identifier fileId = e.getKey();
